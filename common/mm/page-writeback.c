...
static int in_suspend_state;
int dirty_writeback_suspend(void)
{
	unsigned long flags;

	local_irq_save(flags);
	/* prevent multi increase of dirty_writeback_interval variable  */
	if (in_suspend_state == 0) {
		in_suspend_state = 1;
...
EXPORT_SYMBOL(dirty_writeback_suspend);
int dirty_writeback_resume(void)
{
	unsigned long flags;
...
EXPORT_SYMBOL(dirty_writeback_resume);
