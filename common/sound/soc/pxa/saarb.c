...
#define PCM_SSP_PORT	3	  /* used as ap voice port */
...
static int saarb_ssp_pm860x_pcm_startup(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai_link *machine = rtd->dai;
	struct snd_soc_dai *cpu_dai = machine->cpu_dai;
	...
	u32 sscr0, sscr1, sspsp, sstsa, ssrsa;

	cpu_dai->playback.channels_min = 0;
	cpu_dai->playback.channels_max = 0;
	...
	sscr0 = 0xe0c0003f;  /*0xc0d0001f;*/

	/* set codec as clock master (was 0x60001cc1) */
	sscr1 = 0x63301cc1;
	sscr1 &= ~0x03000000;	/* BSSP3 as master for generating clocks */
	sspsp = 0x31810015;	
	sstsa = 0x00000001;
	ssrsa = 0x00000001;
	...
	/* configure GPIOs for ABU/SSI */
	printk("[audio]-->saarb_ssp_pm860x_pcm_startup: configure GPIOs for AP_BSSP3.\n");
	pxa95x_ssp_mfp_init(true);

	return 0;
}

static int saarb_ssp_pm860x_pcm_prepare(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai_link *machine = rtd->dai;
	struct snd_soc_dai *cpu_dai = machine->cpu_dai;
...
static void saarb_ssp_pm860x_pcm_shutdown(struct snd_pcm_substream *substream)
{
	/* configure GPIOs for ABU/SSI */
	printk("[audio]-->saarb_ssp_pm860x_pcm_shutdown: configure GPIOs to CP_GSSP1.\n");
...
{
	.startup = saarb_ssp_pm860x_pcm_startup,
	.prepare = saarb_ssp_pm860x_pcm_prepare,
	.shutdown = saarb_ssp_pm860x_pcm_shutdown,
},
};
...
static struct snd_soc_dai_link saarb_abu_dai_links[] = {
{
	...
	#if defined(CONFIG_SND_SOC_D1981)
	.codec_dai = &d1981_audio_dai[2],
	#else
	.codec_dai = &pm860x_audio_dai[2],
	#endif
...
{
	.name = "SSP-PCM",
	.stream_name = "SSP and Sanremo: PCM voice",
	.cpu_dai = &pxa3xx_ssp_dai[PCM_SSP_PORT -1],
	#if defined(CONFIG_SND_SOC_D1981)
	.codec_dai = &d1981_audio_dai[3],
	#else
	.codec_dai = &pm860x_audio_dai[3],
	#endif
	.ops = &saarb_machine_ops[2],
	.init = saarb_pm860x_init,
}
};
...
},
{
	...
	.platform = &pxa95x_ssp_soc_platform,
