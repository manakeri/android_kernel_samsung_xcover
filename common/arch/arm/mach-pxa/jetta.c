/*
 *  linux/arch/arm/mach-pxa/jetta.c
 *
 *  Support for the Marvell Handheld Platform (aka JETTA)
 *
...
#define JETTA_NR_IRQS	(IRQ_BOARD_START + 40)
...
/* +  OTP device*/
#define OTP_I2C_SDA_GPIO          145
#define OTP_I2C_SCL_GPIO          144


static struct i2c_gpio_platform_data otp_i2c_bus_data = {
	.sda_pin = OTP_I2C_SDA_GPIO,
	.scl_pin = OTP_I2C_SCL_GPIO,
	.udelay  = 3,  //// brian :3
	.timeout = 100,
};

static struct platform_device otp_i2c_bus_device = {
	.name		= "i2c-gpio",
	.id		= 3, /* pxa95x-i2c are bus 0, 1, 2, so start at 3 */
	.dev = {
		.platform_data = &otp_i2c_bus_data,
	}
};

static struct i2c_board_info __initdata otp_i2c_devices[] = {
	{
		I2C_BOARD_INFO("otp_device", 0x33),
             .irq = IRQ_GPIO(mfp_to_gpio(MFP_PIN_GPIO152)), 
	},
};
/* - OTP device */
...
/* + LCD esd check device */
static struct resource lcd_esd_check_resources[] = {
	[0] = {
		.start	= IRQ_GPIO(mfp_to_gpio(MFP_PIN_GPIO16)),
		.end	= IRQ_GPIO(mfp_to_gpio(MFP_PIN_GPIO16)),
		.flags	= IORESOURCE_IRQ,
	}
};

static struct platform_device lcd_esd_check_device = {
	.name 		= "lcd_esd",
	.id 		= -1,
	.num_resources	= ARRAY_SIZE(lcd_esd_check_resources),
	.resource	= lcd_esd_check_resources,
};
/* - LCD esd check device */
...
static void init_tsp(void)
{
	...
	msleep(1);
       printk("[TSP] init_tsp\n");
#if 0
	reset_pin = mfp_to_gpio(MFP_PIN_GPIO48);
	if (gpio_request(reset_pin, "tsp reset"))
...
static void regulator_init(void)
{
	...
	REG_INIT(i++, LDO5, 3100000, 3100000, 0, 0);
...
extern void jetta_lcd_init(void);
...
static int mmc1_wakeup(pm_wakeup_src_t src, int enable)
{
	...
	if (enable) {
		if (src.bits.mmc1_cd) {
			mfp_c = GPIO130_GPIO | MFP_LPM_EDGE_BOTH;
			pxa3xx_mfp_config(&mfp_c, 1);
			ret |= PXA95x_PM_WE_GENERIC(13);
		}
	} else {
		mfp_c = GPIO130_GPIO | MFP_LPM_EDGE_NONE;
		pxa3xx_mfp_config(&mfp_c, 1);
	}
...
void jetta_pxa95x_abu_mfp_init(bool abu)
{
	if (abu)
...
static struct sdhci_pxa_platdata mci1_platform_data = {
	.quirks = SDHCI_QUIRK_NO_ENDATTR_IN_NOPDESC,
	.ext_cd_gpio = mfp_to_gpio(MFP_PIN_GPIO130),
...
static unsigned int matrix_key_map[] = {
	...
	KEY(5, 3, KEY_B),
	KEY(5, 4, KEY_M),
	KEY(5, 5, KEY_QUESTION),
	KEY(5, 6, KEY_ENTER),
      //KEY(5, 7, KEY_DOWN),

      KEY(6, 1, KEY_0),		/* 700 */
	KEY(6, 2, KEY_LEFTALT),       
	KEY(6, 3, KEY_CANCEL),           /* SYM */
	KEY(6, 4, KEY_SPACE),
	KEY(6, 5, KEY_SPACE),
	KEY(6, 6, KEY_DOT),
      //KEY(6, 7, KEY_DOWN),
	...
	KEY(7, 5, KEY_BACK),
	KEY(7, 6, KEY_ZOOM),
      KEY(7, 7, KEY_OK),  /* OTP KEY */
#else
	/* KEY(row, col, key_code) */
	KEY(1, 1, KEY_G),
	KEY(1, 2, KEY_E), 
	KEY(1, 3, KEY_O), 
	KEY(1, 4, KEY_LEFTSHIFT),
	KEY(1, 5, KEY_N), 
	KEY(1, 6, KEY_0), 
	KEY(1, 7, KEY_MENU), 
	...
	KEY(6, 6, KEY_DOT),
      KEY(6, 7, KEY_SEARCH),
       ...
       //KEY(7, 1, KEY_MENU),		
	KEY(7, 2, KEY_VOLUMEDOWN),      
	KEY(7, 3, KEY_VOLUMEUP),
	//KEY(7, 4, KEY_HOME),
	//KEY(7, 5, KEY_BACK),
	//KEY(7, 6, KEY_ZOOM),
      KEY(7, 7, KEY_OK),  /* OTP KEY */
...
#if defined(CONFIG_CAMERA_ISX012)
static struct sensor_platform_data isx012_sensor_data = {
	.id = SENSOR_HIGH,
	.power_set = sensor_power_set,
};
#endif
...
static struct i2c_board_info i2c2_info[] = {  
...
#if defined(CONFIG_CAMERA_ISX012)
    {
    	.type           = "isx012",
    	.addr           = 0x3C, // 0x1A,    
    	.platform_data	= &isx012_sensor_data,
    },
#endif
...
static void gps_power_on(void)
{
	...
	g_gps_regulator = regulator_get(NULL, REGULATOR_GPS);
	if (!g_gps_regulator) {
...
#ifdef CONFIG_PROC_FS
/* SAMSUNG_SSENI : COMM source shoud be modified if you want to modify the below definition. */
/* driver/saarb is used ML8D SP1 COMM. */
/* The below line is used at ML8D SP2 COMM. */
//#define PROC_FILE	"driver/pmic_id"
#define PROC_FILE	"driver/saarb"
...
static void create_proc_file(void)
{
	...
	if (proc_file) {
		...
		printk(KERN_ALERT "GFORCE proc file created!\n");
	} else
		printk(KERN_ALERT "GFORCE proc file create failed!\n");
}
#endif
...
static void __init init(void)
{
	...
	set_abu_init_func(jetta_pxa95x_abu_mfp_init);
	...
	i2c_register_board_info(2, ARRAY_AND_SIZE(i2c3_info));
       ...
       /* + for OTP */
       platform_device_register(&otp_i2c_bus_device);
       i2c_register_board_info(3, ARRAY_AND_SIZE(otp_i2c_devices));
       /* - for OTP */
...
/*+  add for the lcd ESD*/
#ifdef CONFIG_LCD_ESD_CHECK
	platform_device_register(&lcd_esd_check_device);
#endif	
/*- add for the lcd ESD*/
...
*/
MACHINE_START(JETTA_MG2, "PXA968 Handheld Platform (aka JETTA)")
	.phys_io	= 0x40000000,
	.io_pg_offst	= (io_p2v(0x40000000) >> 18) & 0xfffc,

	.map_io         = pxa_map_io,
	.nr_irqs	= JETTA_NR_IRQS,
