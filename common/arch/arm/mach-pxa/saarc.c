 ...
 *
 *  Support for the Marvell Handheld Platform (aka SAARC)
 *
...
#define SAARC_NR_IRQS	(IRQ_BOARD_START + 40)
...
static struct regulator_consumer_supply regulator_supply[] = {
	...
	[PM8607_ID_LDO9] = REGULATOR_SUPPLY("v_mmc", "sdhci-pxa.1"),
	[PM8607_ID_LDO10] = REGULATOR_SUPPLY("v_ldo10", NULL),
	[PM8607_ID_LDO12] = REGULATOR_SUPPLY("v_ldo12", NULL),
	[PM8607_ID_LDO13] = REGULATOR_SUPPLY("v_cam", "pxa95x-camera.0"),
	[PM8607_ID_LDO14] = REGULATOR_SUPPLY("v_gps", "gps_sirf.0"),
};
...
static mfp_cfg_t bssp2_mfp_cfg[] = {
	/* BSSP2 */
	GPIO68_SSP2_RXD,
	GPIO69_SSP2_TXD,
	GPIO71_SSP2_FRM,
...
{
	if (abu)
		pxa3xx_mfp_config(ARRAY_AND_SIZE(abu_mfp_cfg));
	else
		pxa3xx_mfp_config(ARRAY_AND_SIZE(bssp2_mfp_cfg));
}
...
static struct i2c_pxa_platform_data i2c1_pdata = {
	.use_pio = 0,
	.flags =
	    PXA_I2C_HIGH_MODE | PXA_I2C_FAST_MODE | PXA_I2C_USING_FIFO_PIO_MODE,
...
static struct pxa95xfb_mach_info  hdmi_base_info __attribute__((unused)) __initdata = {
	.id                     = "HDMI-base",
...
{
	int pin0 = mfp_to_gpio(MFP_PIN_GPIO139);
	int pin1 = mfp_to_gpio(MFP_PIN_GPIO43);
	...
	if (enable) {
		if (src.bits.mkey) {
			static mfp_cfg_t key_edgeboth_cfg[] = {
				...
				GPIO8_KP_DKIN_4 | MFP_LPM_EDGE_BOTH,
			};
		...
		if (src.bits.mkey) {
			static mfp_cfg_t key_edgenone_cfg[] = {
				...
				GPIO8_KP_DKIN_4 | MFP_LPM_EDGE_NONE,
			};
...
static void mci_setpower(struct device *dev, unsigned int vdd)
{
	...
	if (bitmap && !oldbitmap) {
		printk(KERN_ALERT "mmc power on\n");
		regulator_enable(host->vmmc);
	} else if (!bitmap && oldbitmap) {
		printk(KERN_ALERT "mmc power off\n");
		regulator_disable(host->vmmc);
	}
}

static int
ext_cd_init(void (*notify_func) (struct platform_device *, int state),
		  void *data)
{
	...
	/*
	 * setup GPIO for saarc MMC controller
	 */
...
static int
ext_cd_cleanup(void (*notify_func) (struct platform_device *, int state),
		     void *data)
{
	struct platform_device *pdev = data;
	struct sdhci_pxa_platdata *pdata = pdev->dev.platform_data;
...
#define TOUCHSCREEN_RESET_GPIO				MFP_PIN_GPIO70
static int ssd2531_ts_pins[] = { TOUCHSCREEN_INT_GPIO, TOUCHSCREEN_RESET_GPIO };
...
static struct i2c_board_info i2c3_info[] = {
...

	{
	 ...
	 .irq = gpio_to_irq(mfp_to_gpio(MFP_PIN_GPIO16)),
...
#if defined(CONFIG_SENSORS_CWGD)
	{
	 ...
	 .irq = gpio_to_irq(mfp_to_gpio(MFP_PIN_GPIO111)),
...
{
	int gpio_pin = mfp_to_gpio(MFP_PIN_GPIO39);
...
{
	int gpio_pin = mfp_to_gpio(MFP_PIN_GPIO40);
...
static void create_proc_file(void)
{
	...
	if (proc_file) {
		...
		printk(KERN_ALERT "SAARC proc file created!\n");
	} else
		printk(KERN_ALERT "SAARC proc file create failed!\n");
}
#endif
...
*/
MACHINE_START(SAARC_MG2, "PXA968 (PXA955) Handheld Platform (aka SAARC)")
	.phys_io	= 0x40000000,
	.io_pg_offst	= (io_p2v(0x40000000) >> 18) & 0xfffc,
    .map_io = pxa_map_io,
    .nr_irqs = SAARC_NR_IRQS,
