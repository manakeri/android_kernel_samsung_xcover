...
static void sec_debug_set_upload_cause(enum sec_debug_upload_cause_t type)
{
	...
	{
		if(*upload_cause == UPLOAD_CAUSE_FORCED_CP_UPLOAD) {
			/*SAMSUNG_SMN: Do nothing */
			printk("Forced CP dump \n");
		}
		else
			*upload_cause = type;
	}
...
// + SAMSUNG_SSENI
void sec_debug_dump(void *buf, int err)
{

	sec_debug_dump_stack();	/*SAMSUNG_SMN: moved up, to flush cache.
                                 */
	if (!strcmp(buf, "User Fault"))
		sec_debug_set_upload_cause(UPLOAD_CAUSE_USER_FAULT);
	else if (!strcmp(buf, "Forced panic by samsung_debug\n"))
		sec_debug_set_upload_cause(UPLOAD_CAUSE_FORCED_UPLOAD);
	else if (!strcmp(buf, "EEH emulated panic\n")) {
