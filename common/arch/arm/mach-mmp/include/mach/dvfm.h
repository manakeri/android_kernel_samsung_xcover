...
struct op_info {
	...
	unsigned int		index;
	/*
	 * Store the client ID that is blocking OP.
	 * DVFM_MAX_CLIENT can't exceed the storage limitation.
	 */
	unsigned long		device[8];
};
...
struct dvfm_driver {
	...
	int (*current_core_freq_get)(void *driver_data);
...
extern unsigned long lowpower_devices[8];
extern unsigned long dvfm_devices[8];
...
extern int dvfm_enable_frequncy_change(int);
extern int dvfm_enable_op_name(char *, int);
extern void dvfm_enable_global(int);
extern void dvfm_enable_lowpower(int);
extern void dvfm_disable_global(int);
extern void dvfm_disable_lowpower(int);
extern int dvfm_disable_op_name(char *, int);
extern int dvfm_disable(int);
extern int dvfm_disable_frequncy_change(int);
...
extern void start_profile(int dev_id, unsigned int state, unsigned int start);
...
static inline int dvfm_register(char *name, int *id){return 0;}
static inline int dvfm_unregister(char *name, int *id){return 0;}
