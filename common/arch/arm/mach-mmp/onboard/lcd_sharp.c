...
static int ls035_lcd_power(struct pxa168fb_info *fbi, unsigned int spi_gpio_cs, unsigned int spi_gpio_reset, int on)
{
	int err = 0;
	printk("enter <%s>.......\n", __func__);
#define SCK_PIN 108
#define SDO_PIN 106
	...
	if (on) {
		...
		if (err) {
			printk(KERN_ERR"failed to request GPIO for SHARP LCD RST\n");
			return -1;
		}
		err = gpio_request(spi_gpio_cs, "LCD CS");
		if (err) {
			printk(KERN_ERR"failed to request GPIO for SHARP LCD CS\n");
			return -1;
		}
		...
		if (err) {
			printk(KERN_ERR"failed to request GPIO for SHARP LCD SPICLK\n");
			return -1;
		}
		err = gpio_request(SDO_PIN, "LCD SDO");
		if (err) {
			printk(KERN_ERR"failed to request GPIO for SHARP LCD SDO\n");
			return -1;
		}
...
{
	struct pxa168fb_mach_info *fb = &sharp_lcd_info, *ovly = &sharp_lcd_ovly_info;
