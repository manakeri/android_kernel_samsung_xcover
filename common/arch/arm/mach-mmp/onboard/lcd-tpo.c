...
static int tpo_lcd_power(struct pxa168fb_info *fbi, unsigned int spi_gpio_cs, unsigned int spi_gpio_reset, int on)
{
	...
	if (on) {
		...
		/*err = gpio_spi_send_tpolcd(tpo_spi_cmdon, ARRAY_SIZE(tpo_spi_cmdon), 16);*/
		err = spi_send(fbi, tpo_spi_cmdon, ARRAY_SIZE(tpo_spi_cmdon), spi_gpio_cs);
		if (err) {
			printk("failed to send spi command on\n");
			return -1;
		}
	} else  {
		/*err = gpio_spi_send_tpolcd(tpo_spi_cmdoff, ARRAY_SIZE(tpo_spi_cmdoff), 16);*/
		err = spi_send(fbi, tpo_spi_cmdoff, ARRAY_SIZE(tpo_spi_cmdoff), spi_gpio_cs);
		if (err) {
			printk("failed to send spi command off\n");
			return -1;
		}
...
{
	struct pxa168fb_mach_info *fb = &dkb_tpo_lcd_info, *ovly =&dkb_tpo_lcd_ovly_info;
