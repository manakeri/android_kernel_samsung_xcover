...
static unsigned long extend_timer_delay;

static int yaffs_suspend(struct platform_device *_dev, pm_message_t state)
{
	/* At suspend the timer timeout extend in 10 secondes*/
	extend_timer_delay = 10 * HZ;   /* add ten secondes delay to timer */
	return 0;
}
static int yaffs_resume(struct platform_device *_dev)
{
	extend_timer_delay = 0;
	return 0;
}

static struct platform_driver yaffs_driver = {
	.suspend	= yaffs_suspend,
	.resume	= yaffs_resume,
	.driver	= {
		.owner	= THIS_MODULE,
		.name	= "yaffs",
...
static int yaffs_BackgroundThread(void *data)
{
	...
	while(context->bgRunning){
		...
		expires = expires + extend_timer_delay;
		Y_INIT_TIMER(&timer);
...
static char *yaffs_dump_dev_part0(char *buf, yaffs_Device * dev)
{
	...
	buf += sprintf(buf, "extend_timer_delay....... %u\n", extend_timer_delay);
	buf += sprintf(buf, "\n");
...
static int __init init_yaffs_fs(void)
{
	...
	ret = platform_driver_register(&yaffs_driver);
	if (ret)
		printk(KERN_ERR "%s ERROR yaffs_driver platform_driver_register failed ret = %d !!!\n", __func__, ret);
	return error;
}
