 ...
 */
static struct nand_ecclayout mlc_onenand_oob_128 = {
	.eccbytes	= 64,
	.eccpos		= {
...
static int onenand_page_address(struct onenand_chip *this,int page, int sector)
{
	...
	/*
	*	for 4K page onenand, one page consists of 8 sectors
	*	fsa is 3bit valid, bit15 bit1 bit0
	*/
...
static int onenand_command(struct mtd_info *mtd, int cmd, loff_t addr, size_t len)
{
	...
	if (info->bbm && info->bbm->is_init && info->bbm->search) {
		...
		reloc_block = info->bbm->search(mtd, addr);  /* search will return the offset directly, not the block number any more */
		addr = (reloc_block | 	(addr & ((1 << this->erase_shift) - 1)));
		if (oldaddr != addr)
			pr_debug("relocate block %d to %d\n", (int)(oldaddr >> this->erase_shift), (int)(addr >> this->erase_shift));
	}
					...
					unsigned int block_size)
{
	...
	while (len) {
		...
		if ( region != NULL && addr == region_end) {
			if (!len)
				break;
			region++;
...
int onenand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	case 128:
		if (ONENAND_IS_MLC(this)) {
			this->ecclayout = &mlc_onenand_oob_128;
			mtd->subpage_sft = 0;
		} else {
	...
	mtd->flags = ONENAND_IS_MLC(this)|| ONENAND_IS_4KB_PAGE(this) ? MTD_CAP_MLCNANDFLASH : MTD_CAP_NANDFLASH;
