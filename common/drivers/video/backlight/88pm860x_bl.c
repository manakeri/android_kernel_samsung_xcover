...
static inline int backlight_power_set(struct pm860x_chip *chip, int port, int on)
{
	int ret = -EINVAL;

	switch (port) {
	case PM8606_BACKLIGHT1:
		ret = on ? pm8606_ref_gp_and_osc_get(chip, WLED1_DUTY) :
			pm8606_ref_gp_and_osc_release(chip, WLED1_DUTY);
		break;
	case PM8606_BACKLIGHT2:
		ret = on ? pm8606_ref_gp_and_osc_get(chip, WLED2_DUTY) :
			pm8606_ref_gp_and_osc_release(chip, WLED2_DUTY);
		break;
	case PM8606_BACKLIGHT3:
		ret = on ? pm8606_ref_gp_and_osc_get(chip, WLED3_DUTY) :
			pm8606_ref_gp_and_osc_release(chip, WLED3_DUTY);
		break;
	}
	return ret;
}
