...
int add_sd8x_rfkill_device(int gpio_power_down, int gpio_reset,
	struct mmc_host ***pmmc, rfkill_plat_set_power set_power)
{
        int ret;
...
static int sd8x_power_on(struct sd8x_rfkill_platform_data *pdata, int on)
{
	...
	pr_debug("%s: on=%d\n", __FUNCTION__, on);
	if (gpio_power_down && gpio_request(gpio_power_down, "sd8xxx power down")) {
...
extern unsigned long mmc_detect_change_sync(struct mmc_host *host,
...
static int sd8x_set_block(void *data, bool blocked)
{
	...
	pr_debug("%s: try to set block state of type(%d) as %d, pre_blocked=%d\n",
		__FUNCTION__, sd8x_data->type, blocked, pre_blocked);
	...
	if (pdata->mmc) {
		...
		while (retry) {
			if (0 == mmc_detect_change_sync(pdata->mmc,
				...
				printk(KERN_WARNING "mmc detect has taken %u ms, "
...
static struct rfkill * sd8x_rfkill_register(struct device *parent,
	enum rfkill_type type, char *name, struct sd8x_rfkill_platform_data *pdata)
{
	int err;
