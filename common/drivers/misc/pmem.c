...
static int is_master_owner(struct file *file)
{
	...
	if (master_file && data->master_file == master_file)
		...
		printk(KERN_ERR "pmem: master_file(0x%x) is NULL or not aligned\n",(unsigned int)master_file);
		return 0;
	}
...
static unsigned long pmem_len(int id, struct pmem_data *data)
{
	if (pmem[id].no_allocator) {
		if(data->index >= 0) {
			...
			printk(KERN_ERR "pmem: data->index has error=%d\n",data->index);
			return 0;
		}
	}
	else
...
static void *pmem_start_vaddr(int id, struct pmem_data *data)
{
	...
	if (pmem[id].cached)
		data->vbase = ioremap_cached(addr, len);
	else if (pmem[id].buffered)
		data->vbase = ioremap_wc(addr, len);
	else
		data->vbase = ioremap(addr, len);
	WARN_ON(!data->vbase);
	return data->vbase;
}

static void pmem_unmap_vaddr(struct pmem_data *data)
{
	if (data->vbase != 0)
		iounmap(data->vbase);
	data->vbase = 0;
		  ...
		  unsigned long *vstart, unsigned long *len)
{
	...
	*vstart = (unsigned long)data->vbase;
	up_write(&data->sem);
	return 0;
}
		    ...
		    unsigned int cmd, enum dma_data_direction dir)
{
	...
	get_pmem_addr(file, &addr, (unsigned long*)&vaddr, &plen);
	down_read(&data->sem);
	/*
	       ...
	       int (*release)(struct inode *, struct file *))
{
	...
	printk(KERN_INFO "%s: %d %d init\n", pdata->name, pdata->cached, pdata->buffered);
	if (!pdata->size) {
