...
#define GPIO_PS_OUT	MFP_PIN_GPIO85
...
#if defined(CONFIG_PMIC_D1980)        
		if(prox_regulator == NULL)
		    prox_regulator = regulator_get(switch_cmd_dev, REGULATOR_PROXMITY);
		if(prox_regulator)
		{
#if defined (CONFIG_MACH_JETTA)            
		    regulator_set_voltage(prox_regulator, OUTPUT_3_3V, OUTPUT_3_3V);
#else
		    regulator_set_voltage(prox_regulator, OUTPUT_2_9V, OUTPUT_2_9V);
#endif
...
static void gp2a_chip_init(void)
{
    ...
    if(prox_regulator == NULL)
    {
        ...
        if(prox_regulator == NULL)
        {
            printk(KERN_ERR "[GP2A] Failure get proximity sensor regulator...!\n");
        }
    }
