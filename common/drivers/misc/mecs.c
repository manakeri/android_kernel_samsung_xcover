...
#include <linux/mecs.h>

#define MECS_DEBUG 0

#if MECS_DEBUG
#define MECSDBG(fmt, args...) printk(KERN_INFO fmt, ## args)
#else
#define MECSDBG(fmt, args...)
#endif
...
static int ecs_ctrl_open(struct inode *inode, struct file *file)
{
        printk(KERN_INFO "[MECS] ecs_ctrl_open\n");              
#if 1
...
static int ecs_ctrl_release(struct inode *inode, struct file *file)
{
        printk(KERN_INFO "[MECS] ecs_ctrl_release\n");       
...
{
	printk(KERN_INFO "[MECS] ecompass driver: exit\n");
