...
struct mmc_calib_data {
	int sd_hspeed;
	int delay_idx;
	u32 mfp_toggled; /* bool or row-value */
	u32 mfp_val;
};

/*** The delay values have "optimized order" in retclk_del[]
 0x1000, 0x1001, 0x1002, 0x1003, 0x1004, - 200ps, 400ps,600ps, 800ps, 1000ps
 0x2004, 0x200C, 0x2014, 0x201C, 0x2024, - 1.2ns, 1.4ns,1.6ns, 1.8ps, 2ns
 0x3024, 0x3064, 0x30A4, 0x30E4, 0x3124, - 2.2ns, 2.4ns,2.6ns, 2.8ps, 3ns
 0x4124, 0x4324, 0x4524, 0x4724, 0x4924, - 3.2ns, 3.4ns,3.6ns, 3.8ps, 4ns
***/

static u32 retclk_del[2][RETCLK_DEL_NUM_SUMPLES] =
{
 { /* Slow Speed */
 0x1001,
 0x1000,         0x1002, 0x1003, 0x1004,
...
/*
 * Internal function for RECOVERY and CALIBRATION
 */
static int mmc_getstatus_stuffing(struct mmc_host *host, struct mmc_request **p_mrq)
{
#if !defined(MMC_RECOVERY_WITH_STATUS)
	return 0;
#else
	/* In many CRC-error cases the CMD13 get-status helps to restore
	* normal MMC working. So let's insert it upon error
	*/
	struct mmc_request *mrq_r;
	struct mmc_command *cmd_r;
	struct mmc_command *stop;
	unsigned long flags;

	if (!host->card || !mmc_card_sd(host->card))
		return 0; /* Do for SD only */

	if (host->recovery.mrq != NULL) {
		/* Go back to the original mrq */
		mrq_r = *p_mrq;
		spin_lock_irqsave(&host->lock, flags);
		*p_mrq = host->recovery.mrq;
		host->recovery.mrq = NULL;
		spin_unlock_irqrestore(&host->lock, flags);
		kfree(mrq_r->stop);
		kfree(mrq_r->cmd);
		kfree(mrq_r);
		/* Restore errors to retry real request */
		(*p_mrq)->cmd->error = -EILSEQ;
		if ((*p_mrq)->data)
			(*p_mrq)->data->error = -EILSEQ;
		return 0;
	}
	...
	if (((*p_mrq)->cmd->error != -EILSEQ) &&
		((*p_mrq)->data && ((*p_mrq)->data->error != -EILSEQ)))
		return 0; /* CMD13 could help for CRC error only */

	if ((*p_mrq)->cmd->opcode == MMC_SEND_STATUS)
		return 0; /* This is already CMD13 */

	/* Keep original mrq-copy and build new CMD13_STATUS mrq->cmd */
	cmd_r = NULL;
	stop = NULL;
	if ((mrq_r = kmalloc(sizeof(struct mmc_request), GFP_KERNEL)) &&
		(cmd_r = kmalloc(sizeof(struct mmc_command), GFP_KERNEL)))
		stop = kmalloc(sizeof(struct mmc_command), GFP_KERNEL);
	if (!stop)
		return 0;
	
	memset(cmd_r, 0, sizeof(struct mmc_command));
	memset(stop, 0, sizeof(struct mmc_command));
	cmd_r->opcode = MMC_SEND_STATUS;
	cmd_r->arg = (*p_mrq)->cmd->arg;
	cmd_r->flags = MMC_RSP_R1 | MMC_CMD_AC;
	(*p_mrq)->cmd->error = 0;
	if ((*p_mrq)->data)
		(*p_mrq)->data->error = 0;

	memcpy(mrq_r, (*p_mrq), sizeof(struct mmc_request));
	host->recovery.mrq = (*p_mrq);
	mrq_r->data = NULL;
	mrq_r->cmd = cmd_r;
	mrq_r->stop = stop;
	spin_lock_irqsave(&host->lock, flags);
	*p_mrq = mrq_r;
	spin_unlock_irqrestore(&host->lock, flags);
	pr_debug("%s: CRC-err; stuffing CMD13\n",
		mmc_hostname(host));
	return 1;
#endif/*MMC_RECOVERY_WITH_STATUS*/
...
* But let's try to adjust
* RETURN:
* -1  - ultimative calibration end
*       calibration cannot be done or completely finished
*  0  - slow clock or another "external" option
*       may be tried by Calling-procedure
*  1  - Calling-procedure should RE-try the Request
*/
static int mmc_recovery_calib(struct mmc_host *host, struct mmc_request *mrq,
					   int state, struct mmc_calib_data *calib_data)
{
	...
	if (!host->card)
		return -1;
	if (!mmc_card_sd(host->card) && !mmc_card_sdio(host->card))
		return -1;
	...
	*	return -1; */
	...
	if (state == 0) { /*init BEFORE sending to Low-Driver */
		...
		calib_data->sd_hspeed = (host->recovery.clk > 25000000);
		calib_data->delay_idx = 0;
		calib_data->mfp_toggled = 0;
		calib_data->mfp_val = 0;
		return 0;
	}
	...
	/* (state == 1) */
	if (!calib_data->mfp_val) /* juts read for debug print */
		calib_data->mfp_val = sdhci_pxa_mfpr_drive(pxa->pdata, 0xFFFFFFF0);

	delay_idx = calib_data->delay_idx; /* current */

	if (delay_idx >= RETCLK_DEL_IDX_MAX) {
		delay_idx = 0;
		 /* For SDIO don't change MFPR
		 * don't change the frequency
		 * just say "finished" by -1
		 */
	...
	/* Finally set MFPR and MM4_RETCLK_DEL */
	delay = 0x20000000 | retclk_del[calib_data->sd_hspeed]
						[delay_idx];
	mrq->cmd->error = 0;
	...
	calib_data->delay_idx = ++delay_idx;
	return 1;
}/*mmc_recovery_calib*/
...
void mmc_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (err && cmd->retries) {
		...
		if (!(host->caps & MMC_CAP_DISABLE_BUS_CLK_GATING))
...
void mmc_wait_for_req(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	do {
		...
		if(mmc_getstatus_stuffing(host, &mrq))
			continue;

		if (!(mrq->cmd->error || (mrq->data && mrq->data->error)) &&
			(mmc_host_clk_rate(host) >= 24000000)) {
			/*no errors*/
			host->recovery.clk_ok_cntr++;
			host->recovery.clk_ok_cntr &= 0x1F; /*wrap-limit*/

			if (host->recovery.clk_ok_cntr >= 10*MMC_THRESHOLD_CLK_OK)
				host->recovery.out_of_service_cntr = 0;
			if (host->recovery.clk_ok_cntr >= MMC_THRESHOLD_CLK_OK)
				host->recovery.severe_err_cntr = 0;
			break; /* exit with OK */
		}
		...
		if ((mrq->cmd->error == -ENOMEDIUM) ||
			(mrq->data && mrq->data->error == -ENOMEDIUM))
			break;

		if (mmc_host_clk_rate(host) < 12000000)
			break; /* "valid" error on recognition */

		/* ========= Error happens ========== */
		recovery_calib = mmc_recovery_calib(host, mrq, 1, &calib_data);

		if (recovery_calib < 0) {
			printk(KERN_WARNING "%s: error! clock %d. Exit with error\n",
				mmc_hostname(host), mmc_host_clk_rate(host));
		...
		*/
		if (host->recovery.clk == 0)
			host->recovery.clk = mmc_host_clk_rate_nonzero(host); /*save orig*/

		if ((host->recovery.clk <= 25000000) ||  /* orig is 24MHz, nothing helps */
			(mmc_host_clk_rate(host) <= 25000000)) {
			host->recovery.severe_err_cntr++;
			printk(KERN_WARNING "%s: error! clock %d slow. Exit with error (severe %d)\n",
				mmc_hostname(host), mmc_host_clk_rate(host), host->recovery.severe_err_cntr);
			break;
		}

		mmc_set_clock(host, mmc_host_clk_rate_nonzero(host)/2);
		printk(KERN_WARNING "%s: error! clock %d down to %d\n",
			mmc_hostname(host), host->recovery.clk, mmc_host_clk_rate(host));

		mmc_recovery_calib(host, mrq, 2/*reset state*/, &calib_data);
...
void mmc_set_data_timeout(struct mmc_data *data, const struct mmc_card *card)
{
	...
	if (mmc_card_sd(card)) {
		...
		timeout_us += data->timeout_clks * 1000 /
			(mmc_host_clk_rate_nonzero(card->host) / 1000);
...
/* For Production Mode: force the system enter to suspend  */
void mmc_froce_unlock(void)
{
	if (wake_lock_active(&mmc_delayed_work_wake_lock)) {
		printk(KERN_WARNING "%s %s mmc_delayed_work_wake_lock is active\n", __FILE__, __FUNCTION__);
		wake_unlock(&mmc_delayed_work_wake_lock);
	}
}
