...
static void rndis_set_first_interfrace(struct usb_function *f, unsigned intf)
{
	struct f_rndis		*rndis = func_to_rndis(f);
	rndis->ctrl_id = intf;
	rndis->data_id = intf+1;
}
...
rndis_bind_config(struct usb_configuration *c, u8 ethaddr[ETH_ALEN])
{
	...
	rndis->port.func.set_first_interfrace = rndis_set_first_interfrace;
#endif

#ifdef CONFIG_USB_ANDROID_RNDIS
	/* start disabled */
	rndis->port.func.disabled = 0;
#endif
