...
#define ACTIVE_MAX_RETRIES      6
int active_on_complete_cnt;
    ...
    do 
    {
        ...
        *************************************************************/
	    ...
	    uint_32 virtAddr = (uint_32) ((uint_8*)xd_ptr->WSTARTADDRESS + curr_offset);
            ...
            virtAddr += 4096;
	    physAddr = USB_virt_to_phys((void *)virtAddr);
   ...
   **************************************************************/
	...
	USB_printf("%03d: Add not empty: bit_pos=0x%x, prime=%x, status=0x%x, qh->sts=0x%x\n",
        ...
        while(read_safe == FALSE)
        {
            ...
            if(timeout <= 0)
            {
                printk(KERN_DEBUG "%s: Timeout for ATDTW_TRIPWIRE reg = 0x%x\n", __FUNCTION__, 
        ...
        *********************************************************/
		...
		if (xd_ptr->BDIRECTION == ARC_USB_SEND) {
			...
			VUSB20_EP_TR_STRUCT_PTR curr_dtd = (VUSB20_EP_TR_STRUCT_PTR)USB_32BIT_LE(
				USB_DTD_PHYS_TO_VIRT(usb_dev_ptr, ep_queue_head_ptr->CURR_DTD_PTR));

			timeout = 1000;
			while ((curr_dtd->SIZE_IOC_STS & VUSBHS_TD_STATUS_ACTIVE) && (timeout-- >= 0)) {
				if (timeout <= 500)
				printk(KERN_DEBUG "%s line %d\n", __func__, __LINE__);
			}
			if (timeout <= 0) {
				printk(KERN_ALERT "\n===============previous dtd still active???\n");
				printk(KERN_ALERT "ep %d bit_pos %x qh->cur %x nxt %x\n", xd_ptr->EP_NUM, bit_pos,
					ep_queue_head_ptr->CURR_DTD_PTR, ep_queue_head_ptr->NEXT_DTD_PTR);
					dump_ep_req(xd_ptr->EP_NUM, 1);
			}
		} else {
			// xj test
			if (xd_ptr->EP_NUM) {
				...
				temp_ep_stat = USB_32BIT_LE(dev_ptr->REGISTERS.OPERATIONAL_DEVICE_REGISTERS.ENDPTSTATUS)
                                        & bit_pos;
				pr_debug("ep %d re-check 0x%x next_dtd %x\n",
					xd_ptr->EP_NUM, temp_ep_stat, ep_queue_head_ptr->NEXT_DTD_PTR);
				if (!temp_ep_stat) {
					/* check whether it's the last dTD in queue */
					if ((ep_queue_head_ptr->NEXT_DTD_PTR != VUSBHS_TD_NEXT_TERMINATE)) {
						/* not last dTD, prime the Endpoint only */
						dev_ptr->REGISTERS.OPERATIONAL_DEVICE_REGISTERS.ENDPTPRIME = USB_32BIT_LE(bit_pos);
						/* dump_ep_req(1, 0); */
						printk("%s line %d recv ep status 0 while not last dTD, prime again\n",
							__func__, __LINE__);
						goto done;
					}
				}
	...
	if (xd_ptr->BDIRECTION == ARC_USB_SEND) {
		/* FIXME add status check after prime the IN ep */
		static int prime_again;
		VUSB20_EP_TR_STRUCT_PTR curr_dtd = (VUSB20_EP_TR_STRUCT_PTR)USB_32BIT_LE(
			USB_DTD_PHYS_TO_VIRT(usb_dev_ptr, ep_queue_head_ptr->CURR_DTD_PTR));

		timeout = 1000;
		prime_again = 0;
		while ((curr_dtd != first_dTD_ptr) && (timeout >= 0)) {
			curr_dtd = (VUSB20_EP_TR_STRUCT_PTR)USB_32BIT_LE(
				USB_DTD_PHYS_TO_VIRT(usb_dev_ptr, ep_queue_head_ptr->CURR_DTD_PTR));
			timeout--;
			if (timeout <= 500) {
				if (!prime_again) {
					dev_ptr->REGISTERS.OPERATIONAL_DEVICE_REGISTERS.ENDPTPRIME = USB_32BIT_LE(bit_pos);
					prime_again = 1;
					printk(KERN_DEBUG "%s line %d, prime again\n", __func__, __LINE__);
				}
			}
		}
		...
		if (timeout <= 0) {
			...
			printk(KERN_ALERT "ep %d bit_pos %x qh->cur %x nxt %x, first_dTD_ptr %p\n",
				xd_ptr->EP_NUM, bit_pos, ep_queue_head_ptr->CURR_DTD_PTR,
				ep_queue_head_ptr->NEXT_DTD_PTR, first_dTD_ptr);
			/* dump_epin(xd_ptr->EP_NUM); */
		}
	}
   ...
   if (bit_pos) 
   {
   	...
   	dev_ptr->REGISTERS.OPERATIONAL_DEVICE_REGISTERS.ENDPTCOMPLETE = USB_32BIT_LE(bit_pos);
		...
		ARC_DEBUG_TRACE(ARC_DEBUG_FLAG_ISR, " ep_num %d temp %d tr_complete: dTD_ptr %p ep_queue_head_ptr %p\n", 
			...
			if (active_on_complete_cnt >= ACTIVE_MAX_RETRIES) {
			    printk("%s line %d ep %d %s send dTD status keeps active, clear it manually as WA\n",
				__func__, __LINE__, (unsigned)ep_num, direction ? "SEND" : "RECV");
			    dTD_ptr->SIZE_IOC_STS &= 0xffffff00; /* clear the status feild! -- WA */
			} else
			   break;
		     } else
			active_on_complete_cnt = 0;
