...
static struct usb_function *composite_intf_to_func(struct usb_composite_dev *\
			cdev, u8 intf)
{
	struct usb_function *f;
	unsigned i, active_intf = 0;
		...
		enum usb_device_speed speed, void *buf, u8 type)
{
	...
	list_for_each_entry(f, &config->functions, list) {
		...
		dest = next;
#ifdef CONFIG_DYNAMIC_INTERFACE_DESC_INDEX
		firstInterface = 1;	/* flag to indicate we're dealing with
					first interface of specific function */
#endif
...
#ifdef CONFIG_DYNAMIC_INTERFACE_DESC_INDEX
				if (firstInterface) {
					if (f->set_first_interfrace)
						f->set_first_interfrace(f,
							interfaceCount);
					firstInterface = 0;
				}
#else
...
//dh0318.lee 110715 fix issue that wrong interface num is assigned to callmgmt_desc and union_desc for CDC-ACM
#ifdef CONFIG_DYNAMIC_INTERFACE_DESC_INDEX
			} else {
				struct usb_interface_assoc_descriptor *iad;
				struct usb_cdc_union_desc *cdc_union_desc;
				struct usb_cdc_call_mgmt_descriptor *cdc_callmgmt_desc;

				iad = (struct usb_interface_assoc_descriptor *)dest;
				cdc_union_desc = (struct usb_cdc_union_desc *)dest;
				cdc_callmgmt_desc = (struct usb_cdc_call_mgmt_descriptor *)dest;

				if (iad->bDescriptorType == \
					USB_DT_INTERFACE_ASSOCIATION)
					iad->bFirstInterface = interfaceCount;
				else if ((cdc_callmgmt_desc->bDescriptorType == USB_DT_CS_INTERFACE) &&
					(cdc_callmgmt_desc->bDescriptorSubType == USB_CDC_CALL_MANAGEMENT_TYPE))
					cdc_callmgmt_desc->bDataInterface = interfaceCount;
				else if ((cdc_union_desc->bDescriptorType == USB_DT_CS_INTERFACE) &&
					(cdc_union_desc->bDescriptorSubType == USB_CDC_UNION_TYPE)) {
					cdc_union_desc->bMasterInterface0 = interfaceCount-1;
					cdc_union_desc->bSlaveInterface0 = interfaceCount;
				}				
#endif
			}
			dest += intf->bLength;
...
#else
	c->bNumInterfaces = interfaceCount - hDisabledInterfaces;
#endif
	return len;
}
		...
		const struct usb_ctrlrequest *ctrl, unsigned number)
{
	...
	int			tmp;
#ifdef CONFIG_DYNAMIC_INTERFACE_DESC_INDEX
	unsigned		real_interface_index = 0;
#endif
	if (cdev->config)
		reset_config(cdev);
...
#ifdef CONFIG_DYNAMIC_INTERFACE_DESC_INDEX
		result = f->set_alt(f, real_interface_index, 0);
		real_interface_index++;
#else
...
/*dh0318.lee 111006 fix_for_the_tethering_button
	spin_lock_irqsave(&cdev->lock, flags);
	if (!cdev->connected) {
		cdev->connected = 1;
		schedule_work(&cdev->switch_work);
...
*/
	...
	case USB_REQ_SET_INTERFACE:
		...
		if (!cdev->config || w_index >= MAX_CONFIG_INTERFACES)
			break;
#ifdef CONFIG_DYNAMIC_INTERFACE_DESC_INDEX
		f = composite_intf_to_func(cdev, intf);
#else
...
composite_switch_work(struct work_struct *data)
{
	...
	if (cdev->connected != cdev->sw_connected.state) {
		...
		spin_unlock_irqrestore(&cdev->lock, flags);
		printk("dh0318.lee %s UEVENT connected=%d\n",__func__,connected);
	...
	if (config){
		if(cdev->connected!=0){
		  printk("dh0318.lee %s UEVENT cdev->connected=%d, config=%d\n",__func__,cdev->connected,config->bConfigurationValue);
		  switch_set_state(&cdev->sw_config, config->bConfigurationValue);
        }
	else{/*cdev->connected==0*/
		/*dh0318.lee 111116 usb_uevent It should not be happened*/
    	        printk("dh0318.lee %s UEVENT??? cdev->connected=0, config=%d\n",__func__,config);
		/*Abnormal state. Just notify the USB disconnection to the platform layer*/
		switch_set_state(&cdev->sw_config, 0);	
    }
	}
	else{
		printk("dh0318.lee %s UEVENT cdev->connected=%d, config=0\n",__func__,cdev->connected);
		switch_set_state(&cdev->sw_config, 0);
	}
}
