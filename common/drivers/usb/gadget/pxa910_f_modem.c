...
static struct usb_interface_assoc_descriptor acm_iad_desc = {
	.bLength = sizeof(acm_iad_desc),
...
static struct usb_descriptor_header *acm_fs_function[] = {
	(struct usb_descriptor_header *)&acm_iad_desc,
...
static void acm_disable(struct usb_function *f)
{
	...
	if (acm->modem_state != MODEM_CONTROL_MODE) {
		acm->modem_state = MODEM_CONTROL_MODE;

		atcmd[0] = '\0';
		sprintf(atcmd, "AT+CGACT=0,%d\r\n", acm->cid + 1);
		len = tty_buffer_request_room(tty, strlen(atcmd));
		if (len > 0) {
			tty_insert_flip_string(tty, atcmd, len);
...
static void acm_set_first_interfrace(struct usb_function *f, unsigned intf)
{
	struct f_acm		*acm = func_to_acm(f);
	acm->ctrl_id = intf;
	acm->data_id = intf+1;
...
{
	/*FIXME: Don't do anything here. move this operation to gs_ioctl*/
#if 0
...
static int acm_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	/* init iad */
	acm_iad_desc.bFirstInterface = acm->ctrl_id;
	acm_iad_desc.bInterfaceCount = 0x2;
	acm_iad_desc.bFunctionClass =
	    acm_control_interface_desc.bInterfaceClass;
	acm_iad_desc.bFunctionSubClass =
	    acm_control_interface_desc.bInterfaceSubClass;
	acm_iad_desc.bFunctionProtocol =
	...
	if (gadget_is_dualspeed(c->cdev->gadget)) {
		...
		acm->hs.in = usb_find_endpoint(acm_hs_function,
					       ...
					       &acm_hs_in_desc);
		acm->hs.out =
		    usb_find_endpoint(acm_hs_function, f->hs_descriptors,
				      &acm_hs_out_desc);
		acm->hs.notify =
		    usb_find_endpoint(acm_hs_function, f->hs_descriptors,
				      &acm_hs_notify_desc);
 ...
 */
static int acm_bind_config(struct usb_configuration *c, u8 port_num)
{
	struct f_acm *acm;
	int status;
	...
	acm->port.func.set_first_interfrace = acm_set_first_interfrace;
	...
	if (acm->modem_state != MODEM_CONTROL_MODE) {
		...
		while (count > 0) {
			...
			if (log_count == 100 && port) {
				buf_avail = gs_buf_space_avail(
						&port->port_write_buf);

				printk(KERN_INFO "%s: cannot send the packet \
						over 10s! (%d)\n", \
						__func__, buf_avail);
				log_count = 0;
			}
				  ...
				  const unsigned char *buf, int count)
{
	...
	struct f_acm *acm = port_to_acm(port->port_usb);
	if (acm && acm->modem_state != MODEM_CONTROL_MODE)
		return count;

	return gs_write(tty, buf, count);
}
...
static int __init
gs_marvell_modem_port_alloc(unsigned port_num,
...
int __init marvell_modem_gserial_setup(struct usb_gadget *g, unsigned count)
{
...
int marvell_modem_function_bind_config(struct usb_configuration *c)
{
...
{
	printk(KERN_INFO "f_marvell_modem init\n");
	android_register_function(&marvell_modem_function);
	return 0;
}

module_init(init);

#endif /* CONFIG_USB_ANDROID_PXA910_MODEM */
