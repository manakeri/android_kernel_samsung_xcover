...
#ifdef CONFIG_USB_PXA955_ACM_PPP_MODE

#include <linux/uaccess.h>
#include "pxa955_serial_ppp.h"
...
static int gs_write(struct tty_struct *tty, const unsigned char *buf, \
	int count);
static int gs_write_from_com(struct tty_struct *tty, const unsigned char *buf, int count);
static void gs_read_complete(struct usb_ep *ep, struct usb_request *req);
static void gs_init_data_service(void);

void gs_register_data_service(dataCbFuncs DataCbFuncs)
{
	serial_ppp.cbFuncs = DataCbFuncs;
	gs_init_data_service();
}
EXPORT_SYMBOL(gs_register_data_service);
...
EXPORT_SYMBOL(gs_unregister_data_service);

static int gs_data_rx_callback(const unsigned char *buf, int count, \
	unsigned char cid)
{
	return gs_write_from_com(serial_ppp.tty, buf, count);
}

void	gs_set_modem_control_mode(void)
{
	char deact_pdp[20];
	int buf_len = 0, len;
	...
	if (serial_ppp.modem_curr_cid != 0xFF) {
		...
		sprintf(deact_pdp, "AT+CGACT=0,%d\x0d", \
			serial_ppp.modem_curr_cid+1);

		buf_len = strlen(deact_pdp);

		len = tty_buffer_request_room(serial_ppp.tty, buf_len);
		if (len > 0) {
			tty_insert_flip_string(serial_ppp.tty, deact_pdp, \
				len);
			tty_flip_buffer_push(serial_ppp.tty);
			wake_up_interruptible(&serial_ppp.tty->read_wait);
		}
	} else
		pr_debug("modem_curr_cid = 0xFF\n");
}

static void gs_send_from_tty(char *packet, unsigned int size)
{
	unsigned int len;
	struct gs_port *port = serial_ppp.tty->driver_data;

	if (port == NULL) {
		/* Coverity CID=85*/
		printk(KERN_ERR "gs_send_from_tty: port=NULL port pointer\n");
		return;
	}
	...
	if (port->open_count == 0) {
		printk(KERN_ERR "gs_send_from_tty: port=%d, port is closed\n",
			port->port_num);
		return;
	}


	if (serial_ppp.tty == NULL) {
		printk(KERN_ERR "gs_send_from_tty: port=%d, NULL tty pointer\n",
			port->port_num);
		return;
	}

	len = tty_buffer_request_room(serial_ppp.tty, size);
	if (len > 0) {
		tty_insert_flip_string(serial_ppp.tty, packet, len);
		tty_flip_buffer_push(serial_ppp.tty);
		wake_up_interruptible(&serial_ppp.tty->read_wait);
	} else{
		printk(KERN_ERR "gs_send_from_tty: tty_buffer_request_room returned zero\n");
	}
	spin_unlock(&port->port_lock);
}

void	gs_ppp_connect(unsigned char cid)
{
	char connect_str[30] = "AT+CGDATA=\"NULL\",%d\x0d";
	if (cid != 0xFF) {
		sprintf(connect_str, connect_str, cid+1);
		gs_send_from_tty(connect_str, strlen(connect_str));
	}
}

void	gs_authenticate(unsigned char cid,	\
	PppAuthenticationParamsS *auth_params)
{
	...
	if (auth_params->auth_type == PPP_PAP) {
		if ((cid != 0xFF) && (
			*auth_params->PapAuthenticationParams->Username != '\0')){
			sprintf(auth_str , auth_str_fmt, cid+1, 1,	\
				auth_params->PapAuthenticationParams->Username,\
				auth_params->PapAuthenticationParams->Password);
			gs_send_from_tty(auth_str, strlen(auth_str));
		}
...
static void gs_init_data_service(void)
{
	initParams InitParam = {
		...
		.RxCallbackFunc = gs_data_rx_callback,
		.TerminateCallbackFunc = gs_set_modem_control_mode,
		.AuthenticateCallbackFunc = gs_authenticate,
		.ConnectCallbackFunc = gs_ppp_connect,
	};
	...
	if (serial_ppp.cbFuncs.init) {
		...
		pr_debug("DATA module is not initialized\n");
	...
	unsigned int cmd, unsigned long arg)
{
	...
	ipConnectionParams	dataIpParams;


	if (port == NULL) {
		printk(KERN_ERR PREFIX "NULL port pointer\n");
		return -EIO;
	}

	if (serial_ppp.initialized != ACM_INITIALIZED) {
		printk(KERN_ERR PREFIX "serial_ppp is not initialized\n");
		return -EIO;
	}
	...
	case TIOENABLE:
		...
		pr_debug("TIOENABLE: cid=%ld\n", arg);
		return 0;
	case TIODISABLE:
		serial_ppp.modem_curr_cid = (unsigned char)0xff;
		serial_ppp.cbFuncs.setCid(serial_ppp.modem_curr_cid);
		serial_ppp.modem_state = ACM_CONTROL_MODE;
		pr_debug("TIODISABLE: cid=%ld\n", arg);
		return 0;
	case TIOPPPON:
		pr_debug("TIOPPON: cid=%ld\n", arg);
		...
		gs_write_from_com(tty,
			"\r\nCONNECT\r\n", sizeof("\r\nCONNECT\r\n")-1);

		return 0;
	case TIOPPPOFF:
		pr_debug("TIOPPPOFF: cid=%ld\n", arg);
		serial_ppp.modem_curr_cid = 0xff;
		serial_ppp.cbFuncs.setCid(serial_ppp.modem_curr_cid);
		serial_ppp.modem_state = ACM_CONTROL_MODE;

		/* YYY to do check if following check is reqruired */
		if (port != NULL && port->port_usb != NULL) {
			struct gs_port *port = tty->driver_data;
			struct gserial	*gser;
			...
			pr_debug(\
				"TIOPPPOFF: serial_ppp.cbFuncs.reset()\n");
			serial_ppp.cbFuncs.reset();
		}
		return 0;
	case TIOPPPSETIP:
		...
		pr_debug("TIOPPPSETIP: got IP %08x", \
		...
		void (*fn)(struct usb_ep *, struct usb_request *))
{
	...
	for (i = 0; i < QUEUE_SIZE; i++) {
		req = gs_alloc_req(ep, MAX_REQ_SIZE, GFP_ATOMIC);
		if (!req)
...
static int gs_write_from_com(struct tty_struct *tty, const unsigned char *buf, int count)
{
	...
	if (!port || (serial_ppp.modem_state != ACM_DATA_MODE) || (count == 0))		return count;
	...
	if (port->port_usb) {
		status = txbuf_add((char *)buf, count);
		gs_start_tx(port);
	}
	spin_unlock_irqrestore(&port->port_lock, flags);
	return status*count;
...
static int gs_write(struct tty_struct *tty, const unsigned char *buf, int count)
{
	...
	if (serial_ppp.modem_state == ACM_DATA_MODE || count == 0)
		return count;
...
int __init gserial_setup(struct usb_gadget *g, unsigned count)
{
	...
	coding.dwDTERate = USB_CDC_DTE_RATE_DEFAULT*8;
