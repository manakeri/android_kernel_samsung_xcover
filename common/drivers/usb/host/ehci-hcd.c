...
static void __attribute__((unused)) ehci_port_power(struct ehci_hcd *ehci, int is_on)
{
	unsigned port;
...
static void
__attribute__((unused)) ehci_endpoint_reset(struct usb_hcd *hcd, struct usb_host_endpoint *ep)
{
	struct ehci_hcd		*ehci = hcd_to_ehci(hcd);
	struct ehci_qh		*qh;
...
#include "ehci-hsic.c"
#define HSIC_PLATFORM_DRIVER		mmp3_hsic_driver
#else
