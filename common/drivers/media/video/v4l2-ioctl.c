		...
		unsigned int cmd, void *arg)
{
	...
	/* FIXME: The line below is just for DEBUGing remove it! */
	//printk("__video_do_ioctl command is %d\n", cmd);
	...
	case VIDIOC_S_FLASH:
	{
		struct v4l2_flash *flash = (struct v4l2_flash*)arg;
		
		if (!ops->vidioc_s_flash)
			break;
        
		ret = ops->vidioc_s_flash(file, fh, flash);
		if (!ret)
			dbgarg(cmd, "flash_mode=%d\n", flash->flash_mode );
		break;
	}
    ...
    case VIDIOC_G_FLASH:
	{
		...
		if (!ops->vidioc_g_flash)
			break;

		ret = ops->vidioc_g_flash(file, fh, flash);
		if (!ret)
			dbgarg(cmd, "flash_mode=%d\n", flash->flash_mode );
		break;
	}
	...
	{
		struct v4l2_wb_mode *wb = (struct v4l2_wb_mode*)arg;
		
		if (!ops->vidioc_s_wbmode)
			break;
        
		ret = ops->vidioc_s_wbmode(file, fh, wb);
		if (!ret)
			dbgarg(cmd, "wb_mode_name=%s\n", wb->wb_mode_name );
		break;
	}
    ...
    case VIDIOC_G_WBMODE:
	{
		...
		if (!ops->vidioc_g_wbmode)
			break;

		ret = ops->vidioc_g_wbmode(file, fh, wb);
		if (!ret)
			dbgarg(cmd, "wb_mode_name=%s\n", wb->wb_mode_name );
		break;
	}
	...
	{
		struct v4l2_query_wb_mode *querywbmode = (struct v4l2_query_wb_mode *)arg;
		
		if (!ops->vidioc_querywbmode)
			break;

		ret = ops->vidioc_querywbmode(file, fh, querywbmode);
		if (!ret)
			dbgarg(cmd, "index=%d\n", querywbmode->index );
		break;
	}
	...
	case VIDIOC_S_EFX:
	{
		struct v4l2_efx *efx = (struct v4l2_efx *)arg;
		
		if (!ops->vidioc_s_efx)
			break;
        
		ret = ops->vidioc_s_efx(file, fh, efx);
		if (!ret)
			dbgarg(cmd, "efx_name=%s\n", efx->efx_name );
		break;
	}
    ...
    case VIDIOC_G_EFX:
	{
		...
		ret = ops->vidioc_g_efx(file, fh, efx);
		if (!ret)
			dbgarg(cmd, "efx_name=%s\n", efx->efx_name );
		break;
	}
	...
	{
		struct v4l2_query_efx *queryefx = (struct v4l2_query_efx *)arg;
		
		if (!ops->vidioc_queryefx)
			break;

		ret = ops->vidioc_queryefx(file, fh, queryefx);
		if (!ret)
			dbgarg(cmd, "index=%d\n", queryefx->index );
		break;
	}
