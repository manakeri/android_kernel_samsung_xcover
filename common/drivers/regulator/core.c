...
#if defined(CONFIG_REGULATOR_D1980)
		if(constraints->boot_on)
		{
		    rdev->use_count++;
            printk(KERN_INFO "### [%s] ###  %s >>  constraints->boot_on = %d,  rdev->use_count = %d\n", \
                   rdev->desc->name, __func__, constraints->boot_on, rdev->use_count);
		}
#endif /* CONFIG_REGULATOR_D1980 */
	}
...
#if 0 //defined(CONFIG_REGULATOR_D1980)   //DLG
	list_for_each_entry(rdev, &regulator_list, list) {
