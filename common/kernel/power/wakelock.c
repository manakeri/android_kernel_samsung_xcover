...
int suspend_forbidden = 0;
static DEFINE_SPINLOCK(list_lock);
static DEFINE_SPINLOCK(get_state_lock);
...
static void suspend_func(void)
{
	...
	case TRANS_ACTIVE_TO_SUSPEND:
		...
		if (!error) {
			...
			printk(KERN_ERR "PM: Some processes failed to suspend, start to resume\n");
		if (error) {
			thaw_processes();
	...
	struct wake_lock *lock, long timeout, int has_timeout)
{
	...
	if (suspend_forbidden) {
		printk(KERN_WARNING "%s request  wake lock but suspend_forbidden set\n", lock->name);
		return;
	}
...
#ifdef CONFIG_PXA95x_SUSPEND
	init_MUTEX_LOCKED(&(suspend_thread_struct.sem));
	suspend_thread_struct.current_state = SUSPEND_STATE_ACTIVE;
...
/*  Retrun suspend state */
void get_full_suspend_state(unsigned int *state)
{
	unsigned long flags;
	spin_lock_irqsave(&get_state_lock, flags);
	*state = suspend_thread_struct.current_state;
	spin_unlock_irqrestore(&get_state_lock, flags);
}
