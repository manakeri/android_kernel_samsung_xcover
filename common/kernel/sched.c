...
#if USE_YOKOGAWA_SMT_INT == 1
#define YDC_SMT_DEBUG 
#endif

#ifdef YDC_SMT_DEBUG
#include <smt/SMTAPI.h>
#define YDC_SMT_NAME_LENGTH 256
char proc_name[YDC_SMT_NAME_LENGTH] = "";
#endif /* YDC_SMT_DEBUG */
//#-bired - Yokogawa SMT porting
...
//#+bired - Yokogawa SMT porting
    ...
    if( unlikely(!current->pid)) {
      _SMT_OsSwitch_Idle();
        ...
        int proc_name_user = false;
        if(current->mm != NULL){
            // apllication thread
            int mappingFlg = false;
            unsigned long addr = (unsigned long)(current->active_mm->arg_start);
            ...
            if(mappingFlg){
                proc_name[0]='\0';
                strncpy_from_user(&proc_name[0], (char*)current->active_mm->arg_start, YDC_SMT_NAME_LENGTH);
                if(proc_name[0]!='\0'){
                    proc_name_user = true;
                    _SMT_OsSwitch_ThreadProcess_Name( (unsigned long)(current->pid), (unsigned long)(current->tgid), current->comm, proc_name);
                }
            }
        }
        
        if( !proc_name_user ){
          _SMT_OsSwitch_ThreadProcess_Name( (unsigned long)(current->pid), (unsigned long)(current->tgid), " ", current->comm);
        }
    }
#endif /* YDC_SMT_DEBUG */
//#-bired - Yokogawa SMT porting
