...
static bool read_counter_pj1(int counter_id, unsigned long long *p_value)
{
		                   ...
		                   struct pmu_registers_pj1 *pmu_regs)
{
	...
	pmu_regs->cor[pmn_index] = cs->event_id | 0x1;
...
static int start_mc_counters(void)
{
	...
	result = request_irq(IRQ_DDR_MC, px_mc_irq, IRQF_DISABLED, "CPA MC", dev_id);
	if (result)
		return result;
