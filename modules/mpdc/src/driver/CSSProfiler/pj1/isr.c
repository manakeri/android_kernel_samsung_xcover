...
extern irqreturn_t px_timer_isr(struct pt_regs * const regs, unsigned int pid, unsigned int tid);
extern unsigned int get_timer_irq(void);

extern unsigned int g_pmu_irq_num;
                           ...
                           struct pmu_registers_pj1 *pmu_regs)
{
	...
	pmu_regs->cor[pmn_index] = ec->eventId;
