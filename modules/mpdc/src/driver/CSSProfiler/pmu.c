...
#if 0
static irqreturn_t css_pmu_isr(int irq, void *dev_id)
{
...
int allocate_pmu(void)
{
	...
	if ((ret = pmu_register_isr(prm_client_id, px_css_isr, 0)) != 0)
	{
...
alloc_pmu_err:
	...
	if ((ret = request_irq(g_pmu_irq_num/*IRQ_PMU*/, px_css_isr,
#if defined(LINUX_VERSION_CODE) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 21))
			0//IRQF_DISABLED
#else
			SA_INTERRUPT
