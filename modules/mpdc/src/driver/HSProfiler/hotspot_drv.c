...
struct HSTimerSettings g_tbs_settings;	
struct HSEventSettings g_ebs_settings;	
...
extern struct miscdevice pxhs_sample_d;	
extern struct miscdevice pxhs_module_d;	
extern struct miscdevice pxhs_dsa_d;	
...
DECLARE_WAIT_QUEUE_HEAD(pxhs_kd_wait);
static DEFINE_MUTEX(op_mutex);

struct hs_sampling_op_mach *hs_samp_op;

static void init_func_for_cpu(void)
{
#ifdef PX_CPU_PXA2
	hs_samp_op = &hs_sampling_op_pxa2;
#endif

#ifdef PX_CPU_PJ1
	hs_samp_op = &hs_sampling_op_pj1;
#endif

#ifdef PX_CPU_PJ4
	hs_samp_op = &hs_sampling_op_pj4;
#endif
}
...
{	
	...
	g_dsa = __vmalloc(sizeof(struct px_hotspot_dsa),	
                          GFP_KERNEL,	
	...
	memset(g_dsa, 0, sizeof(struct px_hotspot_dsa));	
...
module_init(px_hotspot_init);	
module_exit(px_hotspot_exit);	
...
{	
	...
	ret = hs_samp_op->start(is_start_paused);
	if (ret != 0)	
	{	
		return ret;	
	}	
...
{	
	...
	cb_result.event_count = hs_samp_op->get_count_for_cal(cb_result.register_id);
...
static int set_tbs_settings(struct HSTimerSettings *settings)	
{	
	...
	{	
		if (copy_from_user(&g_tbs_settings, settings, sizeof(struct HSTimerSettings)) != 0)	
		{	
...
static int set_ebs_settings(struct HSEventSettings *settings)	
{	
	...
	{
		if (copy_from_user(&g_ebs_settings, settings, sizeof(struct HSEventSettings)) != 0)	
		{	
			return -EFAULT;	
		}	
	}	
...
static int px_hotspot_d_ioctl( struct inode *inode, 	
                               struct file *fp, 	
                               unsigned int cmd, 	
                               unsigned long arg)	
{	
	...
	case PX_HS_CMD_RESET_SAMPLE_BUFFER_FULL:
		return reset_sample_buffer_full((bool *)arg);

	case PX_HS_CMD_RESET_MODULE_BUFFER_FULL:
...
static unsigned int px_hotspot_d_poll(struct file *fp, struct poll_table_struct *wait)	
{	
	unsigned int mask = 0;	
	
	poll_wait(fp, &pxhs_kd_wait, wait);	
...
static ssize_t px_hotspot_d_write(struct file *fp, 	
                                  const char *buf, 	
                                  size_t count, 	
                                  loff_t *ppos)	
{	
...
static ssize_t px_hotspot_d_read(struct file *fp, 	
                                 char *buf, 	
                                 size_t count, 	
                                 loff_t *ppos)	
{	
...
static int px_hotspot_d_open(struct inode *inode, struct file *fp)	
{	
	if (g_client_count > 0)
		return -EBUSY;
...
static struct file_operations px_hotspot_d_fops = {	
	.owner   = THIS_MODULE,	
	.read    = px_hotspot_d_read,	
	.write   = px_hotspot_d_write,	
	.poll	 = px_hotspot_d_poll,	
	.ioctl   = px_hotspot_d_ioctl,	
	.open    = px_hotspot_d_open,	
	.release = px_hotspot_d_release,	
};	
	
struct miscdevice px_hotspot_d = {	
	MISC_DYNAMIC_MINOR,	
	"px_hs_d",	
	&px_hotspot_d_fops	
};	
