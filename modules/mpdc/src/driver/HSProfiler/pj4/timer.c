...
/* 
 * PJ4 Hotspot Timer implementation
 */
                         ...
                         unsigned int ts)
{
	...
	if (tbs_running)
	{
		...
		sample_rec.pid	   = pid;
		sample_rec.tid	   = tid;
		sample_rec.registerId = COUNTER_PJ4_OS_TIMER;
...
int start_tbs(bool is_start_paused)
{
	...
	/* set match offset */
	OSMR5 = offset;
	ret = request_irq(IRQ_OST_4_11, px_hotspot_isr, IRQF_TIMER|IRQF_SHARED, "CPA Timer", &dev_oscr);
	if (ret != 0)
	{
