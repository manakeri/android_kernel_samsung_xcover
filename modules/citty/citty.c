...
#define CITTY_TTY_MAJOR		244	/* experimental range */
#define CITTY_TTY_MINORS		3	/* only have 1 device */
...
#define CCTDEV_MAJOR 245   /* dynamic major by default */
#define CCTDEV_NR_DEVS 3    /* ccdev0 through ccdev3 */
#define CCTDEV_QUANTUM 4000
#define CCTDEV_QSET    1000

#define NUM_CITTY_BUF 100 //10
#define CITTY_BUF_SIZE 800//1518 /* this is the max ethernet packet size *///ETH_DATA_LEN//1024

int cctdev_major =   CCTDEV_MAJOR;
int cctdev_minor =   0;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,21)	
	citty_tty_driver->devfs_name = "tts/citty%d";
#endif
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,21)	
	citty_tty_driver->flags =  TTY_DRIVER_REAL_RAW | TTY_DRIVER_NO_DEVFS;
#else
...
int cctdev_init_module(void)
{
	...
	for (i = 0; i < cctdev_nr_devs; i++) {
		...
		init_MUTEX(&cctdev_devices[i].sem);
