...
/*+ Add from ASW3 Android project of Samsung Elec. Co. +*/
#ifdef WRITE_MACADDR
    woal_write_macaddr(priv);
#endif
/*- Add from ASW3 Android project of Samsung Elec. Co. -*/
...
woal_mlan_debug_info(moal_private * priv)
{
    ...
    PRINTM(MERROR, "num_cmd_timeout = %ld\n", info.num_cmd_timeout);
    ...
    PRINTM(MERROR, "num_data_h2c_failure = %ld\n",
           info.num_tx_host_to_card_failure);
    PRINTM(MERROR, "num_cmd_h2c_failure = %ld\n",
           info.num_cmd_host_to_card_failure);
    PRINTM(MERROR, "num_data_c2h_failure = %ld\n",
           info.num_rx_card_to_host_failure);
    PRINTM(MERROR, "num_cmdevt_c2h_failure = %ld\n",
           info.num_cmdevt_card_to_host_failure);
    PRINTM(MERROR, "num_int_read_failure = %ld\n", info.num_int_read_failure);
    PRINTM(MERROR, "last_int_status = %ld\n", info.last_int_status);

    PRINTM(MERROR, "num_event_deauth = %ld\n", info.num_event_deauth);
    PRINTM(MERROR, "num_event_disassoc = %ld\n", info.num_event_disassoc);
    PRINTM(MERROR, "num_event_link_lost = %ld\n", info.num_event_link_lost);
    PRINTM(MERROR, "num_cmd_deauth = %ld\n", info.num_cmd_deauth);
    PRINTM(MERROR, "num_cmd_assoc_success = %ld\n", info.num_cmd_assoc_success);
    PRINTM(MERROR, "num_cmd_assoc_failure = %ld\n", info.num_cmd_assoc_failure);
    PRINTM(MERROR, "cmd_resp_received = %d\n", info.cmd_resp_received);
    PRINTM(MERROR, "event_received = %d\n", info.event_received);

    PRINTM(MERROR, "max_tx_buf_size = %ld\n", info.max_tx_buf_size);
    PRINTM(MERROR, "tx_buf_size = %ld\n", info.tx_buf_size);
    PRINTM(MERROR, "curr_tx_buf_size = %ld\n", info.curr_tx_buf_size);

    PRINTM(MERROR, "data_sent=%d cmd_sent=%d\n", info.data_sent, info.cmd_sent);

    PRINTM(MERROR, "ps_mode=%d ps_state=%ld\n", info.ps_mode, info.ps_state);
    PRINTM(MERROR, "wakeup_dev_req=%d wakeup_tries=%ld\n",
...
woal_tx_timeout(struct net_device *dev)
{
    ...
    PRINTM(MERROR, "%lu : Tx timeout (%ld), bss_index=%d\n",
...
woal_moal_debug_info(moal_private * priv, moal_handle * handle, u8 flag)
{
    ...
    PRINTM(MERROR, "malloc_count = %lu\n", phandle->malloc_count);
    PRINTM(MERROR, "lock_count = %lu\n", phandle->lock_count);
    PRINTM(MERROR, "mbufalloc_count = %lu\n", phandle->mbufalloc_count);
#if defined(SDIO_SUSPEND_RESUME)
    PRINTM(MERROR, "hs_skip_count = %lu\n", phandle->hs_skip_count);
    PRINTM(MERROR, "hs_force_count = %lu\n", phandle->hs_force_count);
#endif
...
#ifdef WRITE_MACADDR
mlan_status
woal_write_macaddr(moal_private * priv)
{
    struct file *fp      = NULL;
    ...
    if(IS_ERR(fpnv))
    {
    	...
    	if(IS_ERR(fp))
	    {
			...
			{
				fp = NULL;
				PRINTM(MERROR,"[WIFI:woal_write_macaddr][1] %s: File open error\n", filepath);
				return -1;
			}
			...
			PRINTM(MINFO, "[WIFI:woal_write_macaddr][1] The Random Generated MAC ID : %s\n", macbuffer);
			...
			if(fp->f_mode & FMODE_WRITE)
			{
			    ...
			    if(ret < 0)
					PRINTM(MERROR, "[WIFI:woal_write_macaddr][1] Mac address [%s] Failed to write into File: %s\n", macbuffer, filepath);
			    else
			   	    PRINTM(MINFO, "[WIFI:woal_write_macaddr][1] Mac address [%s] written into File: %s\n", macbuffer, filepath);
			}
    	...
    	rtn_value = kernel_read(fp, 0, buf, 18);
    }
    else
    {
        /* Reading the MAC Address from .nvmac.info file( the existed file or just created file)*/
    	rtn_value = kernel_read(fpnv, fpnv->f_pos, buf, 18);
    	
    	fp = filp_open(filepath, O_RDWR | O_CREAT, 0666);
    	if(IS_ERR(fp))
    	{
    	    fp = NULL;
    	    PRINTM(MERROR, "[WIFI:woal_write_macaddr] %s: File open error\n", filepath);
    	    return -1;
    	}
        ...
        if(fp->f_mode & FMODE_WRITE)
		{
		    ...
		    if(ret < 0)
				PRINTM(MERROR, "[WIFI:woal_write_macaddr] Mac address [%s] Failed to write into File: %s\n", buf, filepath);
		    else
		   	    PRINTM(MINFO, "[WIFI:woal_write_macaddr] Mac address [%s] written into File: %s\n", buf, filepath);
		}
	...
	if (rtn_value)
		...
		PRINTM(MINFO, "woal_write_macaddr: Reading from the '%s' returns 0 bytes.\n", filepath);
    ...
    if (MLAN_STATUS_SUCCESS != woal_request_set_mac_address(priv)) {
        PRINTM(MERROR, "woal_write_macaddr : Set MAC address failed\n");
    }
	else{
		PRINTM(MINFO, "woal_write_macaddr: MAC Address is overwritten\n");
	}
