...
wlan_get_rd_port(mlan_adapter * pmadapter, t_u8 * pport)
{
    ...
    PRINTM(MIF_D, "wlan_get_rd_port: mp_rd_bitmap=0x%04x\n", rd_bitmap);
    ...
    if (pmadapter->mp_rd_bitmap & CTRL_PORT_MASK) {
        ...
        *pport = CTRL_PORT;
        PRINTM(MIF_D, "wlan_get_rd_port: port=%d mp_rd_bitmap=0x%04x\n", *pport,
        ...
        PRINTM(MIF_D, "port=%d mp_rd_bitmap=0x%04x -> 0x%04x\n",
...
wlan_get_wr_port_data(mlan_adapter * pmadapter, t_u8 * pport)
{
    ...
    PRINTM(MIF_D, "wlan_get_wr_port_data: mp_wr_bitmap=0x%04x\n", wr_bitmap);
    ...
    if (*pport == CTRL_PORT) {
        PRINTM(MERROR,
               "Invalid data port=%d cur port=%d mp_wr_bitmap=0x%04x -> 0x%04x\n",
    ...
    PRINTM(MIF_D, "port=%d mp_wr_bitmap=0x%04x -> 0x%04x\n",
    ...
    if (f_do_rx_aggr) {
        ...
        if (MLAN_STATUS_SUCCESS !=
            pcb->moal_read_data_sync(pmadapter->pmoal_handle, &mbuf_aggr,
                                     (pmadapter->ioport | SDIO_MPA_ADDR_BASE |
                          ...
                          t_u8 port, t_u32 next_pkt_len)
{
    ...
    if (f_send_aggr_buf) {
        ...
        ret = wlan_write_data_sync(pmadapter, &mbuf_aggr,
                                   (pmadapter->ioport | SDIO_MPA_ADDR_BASE |
    ...
    if (sdio_ireg & UP_LD_HOST_INT_STATUS) {
        ...
        while (MTRUE) {
            ...
            if (MLAN_STATUS_SUCCESS !=
                ...
                if (MLAN_STATUS_SUCCESS !=
                    pcb->moal_write_reg(pmadapter->pmoal_handle,
                                        HOST_TO_CARD_EVENT_REG, (cr | 0x04)))
                    PRINTM(MERROR, "write CFG reg failed\n");
