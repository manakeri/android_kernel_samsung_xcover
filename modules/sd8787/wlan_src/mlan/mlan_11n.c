                        ...
                        IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        ...
        if (cfg->param.hw_cap_req)
            cfg->param.htcap_cfg = pmadapter->hw_dot_11n_dev_cap;
        else
                                 ...
                                 IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (ISSUPP_CHANWIDTH40(pmadapter->usr_dot_11n_dev_cap))
        SETHT_MCS32(mcs_set);
...
wlan_fill_ht_cap_tlv(mlan_private * priv, MrvlIETypes_HTCap_t * pht_cap)
{
    ...
    /* Fill HT cap info */
    wlan_fill_cap_info(priv, pht_cap);
