...
/**
 *  @brief Delete tx_pause packets in RA list
 *
 *  @param priv			Pointer to the mlan_private driver data struct
 *  @param ra_list_head	ra list header
 *  @param tid          tid
 ...
 */
static INLINE t_u8
wlan_del_tx_pause_pkts_in_ralist(pmlan_private priv,
                                 mlan_list_head * ra_list_head, int tid)
{
    ...
    ra_list =
        (raListTbl *) util_peek_list(priv->adapter->pmoal_handle, ra_list_head,
                                     MNULL, MNULL);
    while (ra_list && ra_list != (raListTbl *) ra_list_head) {
        if (ra_list->tx_pause && ra_list->total_pkts) {
            if ((pmbuf =
 ...
 */
t_void
wlan_drop_tx_pause_pkts(pmlan_private priv)
{
    int i;
    pmlan_adapter pmadapter = priv->adapter;
    pmadapter->callbacks.moal_spin_lock(pmadapter->pmoal_handle,
    ...
    for (i = 0; i < MAX_NUM_TID; i++) {
        if (wlan_del_tx_pause_pkts_in_ralist
    ...
    for (tid = 0; tid < MAX_NUM_TID; ++tid) {
        ...
        if (ra_list) {
            ...
            PRINTM(MINFO,
                   ...
                   ra_list->ra[4], ra_list->ra[5],
                   new_ra[0], new_ra[1],
                   new_ra[2], new_ra[3], new_ra[4], new_ra[5]);
