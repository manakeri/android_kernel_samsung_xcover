	...
	)
{
     ...
     * mb = dsb() + outer_sync();
     * currently outer_sync(cache flush) will do nothing in our kernel,
     * so mb is just same as dsb
     */
	mb();
	...
	)
{
    ...
    {
        gcmkLOG_WARNING_ARGS("Signal destroy failure, it should be unmapped first.");
        gcmkTRACE_ZONE(gcvLEVEL_ERROR,
    		gcvZONE_OS,
                 ...
                 )
{
    ...
    if (signal->signalType == gcvSIGNAL_STALL)
    {
        status = gckOS_WaitSignalNoInterruptible(Os, Signal, Wait);
    }
    else
    {
	...
	)
{
	...
	if (signal == gcvNULL)
	{
        gcmkLOG_WARNING_ARGS("Error -> signal is NULL");
		gcmkTRACE_ZONE(gcvLEVEL_ERROR,
			gcvZONE_OS,
	...
	)
{
	...
	if (signal == gcvNULL)
	{
        gcmkLOG_WARNING_ARGS("Signal is NULL");
		gcmkTRACE_ZONE(gcvLEVEL_ERROR,
			gcvZONE_OS,
	...
	)
{
    ...
    if(disableClk)
    {
        ...
        static struct clk * clkAXI = NULL;
#endif
        static struct clk * clkGC = NULL;

        if(count == 0)
        {
            clkGC = clk_get(NULL, "GCCLK");
            CLOCK_VERIFY(clkGC);
        }
        clk_disable(clkGC);
