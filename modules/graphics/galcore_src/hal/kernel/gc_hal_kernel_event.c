	...
	)
{
	...
	acquired = gcvTRUE;
    if((Event->freeCount > 0) && (Event->freeList != gcvNULL))
    {
	*Record           = Event->freeList;
    ...
    {
        gcmkLOG_ERROR_ARGS("Can't get free event record. *Record=0x%08x, freeCount=%d, status=%d", 
        				*Record, Event->freeCount, status);
		*Record = gcvNULL;
